...
--The delivered filter is applied here because the delay-to-review comparison only makes sense for orders that were actually delivered.
--Grouping by review_score returns the compact five-row score-to-delay summary the dashboard needs, instead of one row per order.
